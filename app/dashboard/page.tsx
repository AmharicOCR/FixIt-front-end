    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Track, manage and resolve your code errors.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[
          { title: "Total Errors", value: "128", icon: Bug, color: "bg-blue-500/20", textColor: "text-blue-500" },
          { title: "Resolved", value: "86", icon: CheckCircle2, color: "bg-green-500/20", textColor: "text-green-500" },
          { title: "Pending", value: "42", icon: Clock, color: "bg-amber-500/20", textColor: "text-amber-500" },
          { title: "Critical", value: "7", icon: AlertCircle, color: "bg-red-500/20", textColor: "text-red-500" },
        ].map((item, index) => (
          <motion.div
            key={index}
          ...
          >
            <Card className="h-full overflow-hidden border-border/40 shadow-sm">
              <CardContent className="p-6 flex items-center gap-4">
                <div className={`flex items-center justify-center rounded-full ${item.color} h-12 w-12`}>
      ...
      <Tabs defaultValue="overview" className="space-y-4" onValueChange={setActiveTab}>
        <TabsList className="grid w-full max-w-lg grid-cols-3 rounded-lg">
          ...
          <TabsTrigger value="assigned-to-me" className="rounded-lg">
            Assigned to Me
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            ...
            <Card className="col-span-4 border-border/40 shadow-sm h-full">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-base font-medium">Error Activity</CardTitle>
                    ...
                    <div key={index} className="flex-1 space-y-2 flex flex-col items-center justify-end">
                      ...
                      >
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-foreground text-background rounded px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {index === 4 && "Fri"}
                          {index === 5 && "Sat"}
                          {index === 6 && "Sun"}: {Math.floor((height / 100) * 20)} errors
                        </div>
                      </div>
            ...
            <Card className="col-span-3 border-border/40 shadow-sm">
              ...
              <CardContent className="p-6">
                <div className="space-y-6">
                  {[
                    { category: "Syntax Errors", percentage: 32, color: "bg-blue-500" },
                    { category: "Runtime Exceptions", percentage: 28, color: "bg-purple-500" },
                    { category: "Network Issues", percentage: 20, color: "bg-green-500" },
                    { category: "Database Errors", percentage: 12, color: "bg-yellow-500" },
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card className="border-border/40 shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-medium">Popular Solutions</CardTitle>
                <CardDescription>Most upvoted solutions this month</CardDescription>
              </CardHeader>
                    ...
                    <div key={i} className="flex items-center justify-between py-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">by {solution.user}</p>
                        </div>
                      </div>
            ...
            <Card className="border-border/40 shadow-sm">
              ...
              <CardContent className="px-6">
                <div className="grid grid-cols-2 gap-4">
                  <Button variant="outline" className="h-auto justify-start gap-2 p-3 text-sm rounded-lg" asChild>
                    <Link href="/dashboard/new-error">
                      <PlusCircle className="h-5 w-5 text-primary" />
                      <div className="text-left">
                        <div className="font-medium">Log Error</div>
                        <div className="text-xs font-normal text-muted-foreground">Report a new issue</div>
                      </div>
                    </Link>
                  </Button>
                  <Button variant="outline" className="h-auto justify-start gap-2 p-3 text-sm rounded-lg" asChild>
                    <Link href="/dashboard/search">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs font-normal text-muted-foreground">Find solutions</div>
                      </div>
                    </Link>
                  </Button>
                  <Button variant="outline" className="h-auto justify-start gap-2 p-3 text-sm rounded-lg" asChild>
                    <Link href="/dashboard/teams">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs font-normal text-muted-foreground">Team workspace</div>
                      </div>
                    </Link>
                  </Button>
                  <Button variant="outline" className="h-auto justify-start gap-2 p-3 text-sm rounded-lg" asChild>
                    <Link href="/dashboard/help">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs font-normal text-muted-foreground">Support resources</div>
                      </div>
                    </Link>
                  </Button>
                </div>
          ...
          <Card className="border-border/40 shadow-sm">
            <CardHeader className="px-6 py-5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base font-medium">Recently Logged Errors</CardTitle>
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 ml-14 sm:ml-0 mt-2 sm:mt-0">
                      <div className="flex items-center text-xs text-muted-foreground">
                        <Avatar className="h-6 w-6 mr-2">
                          ...
                          />
                          <AvatarFallback>{error.reportedBy.initials}</AvatarFallback>
                        </Avatar>
                        <span className="sr-only sm:not-sr-only">{error.reportedAt}</span>
                      </div>
                      <DropdownMenu>
        ...
        <TabsContent value="assigned-to-me" className="space-y-4">
          <Card className="border-border/40 shadow-sm">
            <CardHeader className="px-6 py-5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base font-medium">Errors Assigned to Me</CardTitle>
                <div className="flex items-center gap-2">
                  <Select defaultValue="all">
                    <SelectTrigger className="w-[120px] h-8 text-xs">
                      <SelectValue placeholder="Status" />
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 ml-14 sm:ml-0 mt-2 sm:mt-0">
                      <div className="flex items-center text-xs text-muted-foreground">
                        ...
                        <span className="sr-only sm:not-sr-only">Assigned {error.assignedAt}</span>
                      </div>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
